...
//    // 추가적인 Material 관련 메서드가 필요한 경우 여기에 추가할 수 있습니다.
    ...
    @Query("SELECT b FROM Material b WHERE (:materialName IS NULL OR LOWER(b.materialName) LIKE LOWER(CONCAT('%', :materialName, '%'))) AND (:materialCategory IS NULL OR b.materialCategory = :materialCategory) AND (:materialCategoryExclude IS NULL OR b.materialCategory != :materialCategoryExclude) ORDER BY b.materialId ASC")
    Page<Material> findAllWithkeywordWithcategory(
            @Param("materialName") String materialName,
            @Param("materialCategory") Integer materialCategory,
            @Param("materialCategoryExclude") Integer materialCategoryExclude,
            Pageable pageable
    );
